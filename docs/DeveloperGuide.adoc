...
Note the behaviour of useTaskHistoryTaskId when all other commands (*not* `edit`/`completed`/`addtag`/`delete`) is called (Figure 20). `CommandHistory#addHistoryTaskId()` will not be called thus `userInputHistoryTaskId` will not be updated.
...
Note the behaviour when a `edit` command  is called (Figure 21). `CommandHistory#addHistoryTaskId()` will be called thus `userInputHistoryTaskId` will be updated with the taskId of the task where the
...
. Sorts task in current project by deadline

.. Prerequisites:
    * There sould at least be 2 tasks in the project
    * Tasks should not be sorted by deadline
.. Test case: `sort` +
   Expected: Tasks should be sorted according to deadline i.e. task with the deadline closest to current date should be first
